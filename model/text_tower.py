...
class Text_Tower(nn.Module):
    ...
    def __init__(self,
                ...
                random_truncate_prob : float = 0.5,
                ):
        super().__init__()
        if pubmedbert_checkpoint:
            ...
            self.tokenizer = MyTokenizer(pubmedbert_checkpoint, max_text_length, random_truncate_prob)
        else:
        ...
        if medcpt_checkpoint:
            ...
            self.tokenizer = MyTokenizer(medcpt_checkpoint, max_text_length, random_truncate_prob)
        else:
            self.medcpt = None
        ...
        if biolord_checkpoint:
            ...
            self.tokenizer = MyTokenizer(biolord_checkpoint, max_text_length, random_truncate_prob)
        else:
            self.biolord = None
    ...
    def forward(self, text):
        try:
            text = self.tokenizer.tokenize(text, random_truncate=True) # (n, max_l)
